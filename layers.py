...
class ThetaLayer(lasagne.layers.MergeLayer):
    ...
    def get_output_for(self, input, **kwargs):
        ...
        kltermFn = theano.function([logsigma_in, mu_in],
                                   0.5 * (1 + T.mul(logsigma_in, 2) - (mu_in ** 2) - (T.exp(logsigma_in) ** 2)))
        self.klterm = (0.5 * (1 + logsigma_in * 2) - (mu_in ** 2) - (np.e ** (logsigma_in) ** 2))
        out = theano.function([mu_in, logsigma_in, input],
                              (1 / (input * (T.exp(logsigma_in) * (2 * np.pi) ** (1 / 2)))) * T.exp(
                                  -((T.log(input) - mu_in) ** 2) / (2 * (T.exp(logsigma_in) ** 2))))
...
class ExpressionLayer(lasagne.layers.Layer):
    def __init__(self, expression, output_shape, **kwargs):
        self.expression = expression
        self.output_shape_ = output_shape
        super(ExpressionLayer, self).__init__(**kwargs)
